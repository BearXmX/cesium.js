...
const autoRevolutionTime = 20 // 20s一圈

/* 自转一圈时间 */
export const autoRevolutionTimeInit = (revolutionTimeInit / 365) * 10 * autoRevolutionTime // 0.1秒/圈 * 50 = 5s一圈
...
export const makeOrbit = (scene: THREE.Scene) => {
  const orbitGeometry = new THREE.RingGeometry(staticConfig.radius - 0.05, staticConfig.radius + 0.1, 128)
...
export const createDebugLatLonSphere = (earthRadius: number, earthGroup: THREE.Group) => {
  ...
  // 北极点
  const northPoleMarker = new THREE.Mesh(new THREE.SphereGeometry(poleMarkerSize, 16, 16), new THREE.MeshBasicMaterial({ color: 'red' }))
  northPoleMarker.position.y = actualRadius
  northPoleMarker.name = 'north-pole-marker'
  linesGroup.add(northPoleMarker)

  // 南极点
  const southPoleMarker = new THREE.Mesh(new THREE.SphereGeometry(poleMarkerSize, 16, 16), new THREE.MeshBasicMaterial({ color: 'red' }))
